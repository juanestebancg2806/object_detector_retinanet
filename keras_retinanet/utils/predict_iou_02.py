...
def predict(
    ...
    for i in range(generator.size()):
        ...
        list_filtered_boxes_per_class = [[] for _ in range(soft_scores_old.shape[-1])]
        list_filtered_scores_per_class = [[] for _ in range(soft_scores_old.shape[-1])]
        list_filtered_labels_per_class = [[] for _ in range(soft_scores_old.shape[-1])]
        list_filtered_labels_prob_per_class = [[] for _ in range(soft_scores_old.shape[-1])]
        ...
        for _, detection in filtered_data.iterrows():
            ...
            sel = [ detection['x1'], detection['y1'], detection['x2'], detection['y2']]
            probs_names = []
            for k in range(soft_scores_old.shape[-1]):
                current_detection = image_detections[image_detections[:, -1] == k]
                other_detection = image_detections[image_detections[:, -1] != k]
                label_prob = get_prob(sel, current_detection, other_detection, k)
                probs_names.append([k,label_prob])
            class_selected = sorted(probs_names, key = lambda x: x[1], reverse = True)[0]
            row.append(class_selected[0])
            row.append(class_selected[1])
            list_filtered_boxes_per_class[class_selected[0]].append(box)
            list_filtered_scores_per_class[class_selected[0]].append(detection['confidence'])
            list_filtered_labels_per_class[class_selected[0]].append('{0:.2f}'.format(detection['hard_score']))
            list_filtered_labels_prob_per_class[class_selected[0]].append(class_selected[1])
        ...
        if save_path is not None:
            ...
            for k in range(soft_scores_old.shape[-1]):
                ...
                draw_detections(raw_image, np.asarray(list_filtered_boxes_per_class[k]), np.asarray(list_filtered_scores_per_class[k]),
                        np.asarray(list_filtered_labels_per_class[k]), color=(0, 0, 255), list_label_prob = list_filtered_labels_prob_per_class[k])

                cv2.imwrite(os.path.join(save_path, '{}_{}.png'.format(i,k)), raw_image)
