...
def predict(
    ...
    for i in range(generator.size()):
        ...
        # run network
        #print("Prediction",model.predict_on_batch(np.expand_dims(image, axis=0)),"\nSize Prediction",len(model.predict_on_batch(np.expand_dims(image, axis=0))))
        ...
        for k in range(soft_scores_old.shape[-1]):
            results = image_detections[image_detections[:, -1] == k]
            ...
            for _, detection in filtered_data.iterrows():
                row = [image_name, detection['x1'], detection['y1'], detection['x2'], detection['y2'],
                   detection['confidence'], detection['hard_score'], k]
            ...
            draw_detections(raw_image, np.asarray(filtered_boxes), np.asarray(filtered_scores),
                        np.asarray(filtered_labels), str(k), color=(0, 0, 255))
